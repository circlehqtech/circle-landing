...
import { ArrowRightIcon, CheckCircle2Icon, CheckIcon, HelpCircleIcon, MapPinIcon, MailIcon, PhoneIcon, SendIcon, ShieldCheckIcon } from 'lucide-react';
...
export function ConsultationPage() {
  ...
  return (
    <div className="relative pb-24">
      ...
      <section className="relative isolate overflow-hidden pt-32 pb-20">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-5 sm:px-8">
          ...
          <motion.p
            ...
            Before we build anything, we look at where your business actually stands. A consultation with Circle HQ isn't a sales call — it's a short, honest conversation to figure out whether AI is the right fix for what you're dealing with, and if so, where to start.
          </motion.p>
        </div>
      </section>
        ...
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8">
          <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16 items-start">
            ...
            <div className="lg:sticky lg:top-28 lg:self-start">
              ...
              <p className="mt-6 max-w-md text-sm sm:text-base leading-relaxed text-hq-mute">
                A few honest questions worth asking yourself before you book. Answer Yes or No to the statements that sound like your week.
              </p>

              {/* Counter / Indicator matching screenshot */}
              <div className="mt-10 flex items-baseline gap-3 pt-6 border-t border-hq-line/60">
                <span className="font-mono text-5xl sm:text-6xl font-extrabold text-hq-red leading-none">
                  {yesCount}
                </span>
                <span className="font-mono text-xs uppercase tracking-[0.2em] text-hq-mute font-medium">
                  / {QUESTIONS.length} YESES
                </span>
              </div>
            </div>
            ...
            <div className="space-y-4 sm:space-y-5">
                ...
                return (
                  <motion.div
                    ...
                    className={`group relative rounded-2xl border p-6 sm:p-7 transition-all duration-300 ${
                      isYes 
                        ? 'border-hq-red/70 bg-hq-panel shadow-[0_0_30px_-10px_rgba(224,20,44,0.2)]' 
                        : isNo 
                          ? 'border-hq-line/80 bg-hq-panel/40' 
                          : 'border-hq-line bg-hq-panel/50 hover:border-hq-line/80'
                    }`}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
                      ...
                      <div className="flex items-center gap-2.5 shrink-0 self-start sm:self-center">
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-xs font-mono font-semibold uppercase tracking-wider transition-all duration-200 ${
                            isYes
                              ? 'bg-hq-red text-white shadow-md shadow-hq-red/30'
                              : 'border border-hq-line bg-hq-black/60 text-hq-mute hover:border-hq-red/60 hover:text-white'
                          }`}>
                          {isYes && <CheckIcon size={14} />}
                          Yes
                        </button>
                        <button
                          type="button"
                          ...
                          className={`inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-xs font-mono font-semibold uppercase tracking-wider transition-all duration-200 ${
                            isNo
                              ...
                              : 'border border-hq-line bg-hq-black/60 text-hq-mute hover:border-white/60 hover:text-white'
                          }`}>
                          No
                        </button>
                      </div>
              ...
              <motion.div
                ...
                <p className="text-base sm:text-lg leading-relaxed text-white">
                  {yesCount === 0 ? (
                    'Answer the questions above to assess your business AI readiness. 0 yeses selected so far.'
                  ) : yesCount === 1 ? (
                    '1 yes. There is a clear pattern emerging: operational drag is slowing down your team in key areas. That is fixable.'
                  ) : yesCount >= 2 ? (
                    `${yesCount} yeses. There is a clear pattern here: information and approvals are moving slower than your business is. That is fixable.`
                  ) : (
                    'Your answers indicate clear opportunities where AI and workflow automation will remove operational drag.'
                  )}
                </p>
      ...
      <section id="booking" className="border-t border-hq-line bg-hq-black py-24 sm:py-32">
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8">
                ...
                {formSubmitted ? (
                  <div className="rounded-2xl border border-hq-red/40 bg-hq-black/80 p-8 text-center sm:p-12 space-y-4">
                    ...
                    <p className="text-sm text-hq-mute max-w-md mx-auto">
                      Thank you{formName ? `, ${formName}` : ''}. We have received your details and will get back to you within 24 hours.
                    </p>
                  </div>
                ) : (
