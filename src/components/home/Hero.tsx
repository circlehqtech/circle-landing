...
import { ArrowDownRightIcon, ArrowRightIcon, MousePointerClickIcon } from 'lucide-react';
  ...
  return (
    <section
      ...
      <motion.div
        ...
        <h1 className="font-display text-[13vw] font-semibold leading-[0.95] tracking-[-0.04em] sm:text-[9vw] lg:text-[7.2vw]">
          <span className="block">
            {LINE_ONE.map((word) =>
              <SplitWord key={word.text} word={word} startIndex={nextIndex(word.text.length)} />
            )}
          </span>
          <span className="block">
            {LINE_TWO.map((word) =>
        ...
        <div className="mt-8 grid gap-10 border-t border-hq-line pt-8 lg:grid-cols-[1.1fr_0.9fr]">
          <motion.p
            ...
            Circle HQ builds the AI systems that run your operations and trains the people who’ll manage them.{' '}
            <span className="text-white">Smart AI Solutions & Workforce Capability.</span>
          </motion.p>
...
function SplitWord({ word, startIndex }: { word: Word; startIndex: number; }) {
  ...
  return (
    <span className="mr-[0.22em] inline-block">
      {word.media ? <HoverRevealWord media={word.media}>{chars}</HoverRevealWord> : chars}
    </span>
  );
}
